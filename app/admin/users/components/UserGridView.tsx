...
function UserCard({
  ...
  user: UserGridItem;
  onEdit: (user: UserGridItem) => void;
  onDelete: (id: string) => void;
  onFacialCapture: (user: UserGridItem) => void;
}) {
  return (
    <Card
      variant="interactive"
    ...
    >
      ...
      <div className="absolute top-2 right-2 z-10 opacity-0 group-hover:opacity-100 transition-opacity">
        <CardActionsMenu
          ...
          onFacialCapture={() => onFacialCapture(user)}
        />
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
        {users.map((user) => (
          <UserCard
            ...
            onEdit={onEdit}
            onDelete={onDelete}
            onFacialCapture={onFacialCapture}
          />
        ))}
      </div>
