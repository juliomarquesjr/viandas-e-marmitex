...
export function PreOrderDossier({
  ...
  const [tab, setTab] = useState<Tab>("comanda");
  const stage = stageOf(preOrder);
  const fulfillment = fulfillmentOf(preOrder);
  ...
  return (
    <div className="flex h-full min-h-0 min-w-0 flex-col bg-[color:var(--background)]" style={stageVars(stage)}>
      ...
      {!receiving && (
      <div role="tablist" className="flex gap-0.5 border-b border-[color:var(--border)] bg-[color:var(--card)] px-5">
        <TabButton active={tab === "comanda"} onClick={() => setTab("comanda")}>
          Comanda
        </TabButton>
        ...
        {tab === "comanda" ? (
          <div className="flex flex-wrap items-start gap-6">
            ...
            <div className="min-w-[260px] flex-1">
              <PreOrderTimeline preOrder={preOrder} now={now} />
            </div>
          </div>
        ) : (
      ...
      {receiving ? (
        ...
        <footer className="flex flex-wrap items-center gap-2.5 border-t border-[color:var(--border)] bg-[color:var(--card)] px-5 py-3.5">
          ...
          <Button variant="outline" onClick={onPrint} leftIcon={<Printer className="h-4 w-4" />}>
            Imprimir
          </Button>
          <Button variant="ghost" onClick={onEdit} leftIcon={<Pencil className="h-4 w-4" />}>
            Editar
          </Button>
          <OverflowMenu onTrack={onTrack} onDelete={onDelete} />
          <p className="hidden min-w-0 flex-1 pl-1 text-[11.5px] leading-snug text-[color:var(--muted-foreground)] xl:block">
            {primary?.kind === "receive"
...
function OverflowMenu({ onTrack, onDelete }: { onTrack: () => void; onDelete: () => void }) {
