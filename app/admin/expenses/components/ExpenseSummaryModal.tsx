...
function InfoCard({ label, icon, children, className = "" }: InfoCardProps) {
  return (
    <div className={`rounded-xl p-3 border border-slate-200 flex flex-col gap-2 ${className}`}>
...
export function ExpenseSummaryModal({
  ...
  const handleVerNf = async () => {
    ...
    setNfError(null);
    try {
      const params = new URLSearchParams({ chave: expense.nfChaveAcesso });
      if (expense.nfQrCodeUrl) params.set('url', expense.nfQrCodeUrl);
      const res = await fetch(`/api/nf-scanner/consulta?${params}`);
        ...
        <DialogContent className="sm:max-w-sm gap-0 p-0 overflow-hidden">
          ...
          <div className="bg-white px-5 py-4 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              <InfoCard label="Tipo" icon={<Tag className="h-3.5 w-3.5 text-slate-400" />}>
                <span className="inline-flex items-center text-xs font-medium bg-primary/10 text-primary border border-primary/20 rounded-full px-2 py-0.5">
                  {expense.type.name}
                </span>
              </InfoCard>
              <InfoCard label="Fornecedor" icon={<Truck className="h-3.5 w-3.5 text-slate-400" />}>
                <span className="inline-flex items-center text-xs font-medium bg-slate-100 text-slate-700 border border-slate-200 rounded-full px-2 py-0.5">
            ...
            <InfoCard label="Forma de Pagamento" icon={<CreditCard className="h-3.5 w-3.5 text-slate-400" />}>
              {expense.paymentMethod ? (
                <span className="inline-flex items-center gap-1.5 text-xs font-medium bg-slate-100 text-slate-700 border border-slate-200 rounded-full px-2 py-0.5">
            ...
            {/* Seção: Nota Fiscal (condicional) */}
            {hasNf && (
              <>
                <SectionDivider label="Nota Fiscal" />
                <div className="rounded-xl border border-amber-200 bg-amber-50/40 p-3 space-y-2.5">
                  <div className="flex items-center justify-between">
                    ...
                    <Button
                      ...
                      className="gap-1.5 h-7 px-2.5 text-xs border-amber-200 text-amber-700 hover:bg-amber-100 bg-white"
                    >
                      {isLoadingNf ? (
                  ...
                  <p className="font-mono text-[11px] text-slate-500 break-all leading-relaxed">
                    {maskChave(expense.nfChaveAcesso!)}
                  </p>
                  {expense.nfQrCodeUrl && (
                    <a
                      href={expense.nfQrCodeUrl}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center gap-1 text-[11px] text-amber-600 hover:text-amber-800 hover:underline"
                    >
