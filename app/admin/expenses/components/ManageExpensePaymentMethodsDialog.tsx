...
import { Check, CreditCard, FileText, Wallet, X } from "lucide-react";
...
export function ManageExpensePaymentMethodsDialog({ isOpen, onClose, onChanged }: Props) {
    ...
    const save = async () => {
        try {
            if (!name.trim()) return;
            setSaving(true);
            const url = editing ? `/api/expense-payment-methods/${editing.id}` : "/api/expense-payment-methods";
            ...
            if (!res.ok) throw new Error(data?.error || "Falha ao salvar método de pagamento");
    ...
    const removeItem = async (it: ExpensePaymentMethod) => {
        try {
            ...
            const data = await res.json();
            if (!res.ok) throw new Error(data?.error || "Falha ao remover método");
            ...
            <div className="w-full max-w-3xl max-h-[95vh] overflow-hidden bg-white shadow-2xl rounded-2xl border border-gray-200 flex flex-col">
                <div className="bg-gradient-to-r from-orange-50 to-amber-50 border-b border-gray-200 sticky top-0 z-20 relative p-6">
                    ...
                    <div className="relative flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-gray-600 mt-1 text-sm">Crie, edite e remova formas de pagamento para despesas</p>
                        </div>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-4">
                    <div className="flex items-center justify-between">
                        <div className="text-sm text-gray-600">{items.length} forma(s) cadastrada(s)</div>
                        <Button size="sm" onClick={openCreate} className="bg-orange-600 hover:bg-orange-700 text-white rounded-xl shadow-sm">Nova Forma de Pagamento</Button>
                    </div>
                                ...
                                <div className="flex items-center justify-between p-4 bg-orange-50 rounded-xl border border-orange-200">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm text-gray-600">Ative ou desative esta forma</p>
                                        </div>
                                    </div>
                                    <Button
                                        type="button"
                                        ...
                                        className={`relative h-6 w-11 rounded-full border-2 transition-colors ${active
