...
export function ManageSupplierTypesDialog({ isOpen, onClose, onChanged }: Props) {
  const [items, setItems] = useState<SupplierType[]>([]);
  const [loading, setLoading] = useState(false);
  const [saving, setSaving] = useState(false);
  const [formOpen, setFormOpen] = useState(false);
  const [editing, setEditing] = useState<SupplierType | undefined>();
  ...
  const openEdit = (it: SupplierType) => {
    setEditing(it);
    setName(it.name);
  ...
  const save = async () => {
    try {
      if (!name.trim()) return;
      setSaving(true);
      const url = editing ? `/api/supplier-types/${editing.id}` : "/api/supplier-types";
      ...
      showToast(e?.message || "Erro ao salvar tipo de fornecedor", "error");
    } finally {
      setSaving(false);
    }
  };

  const removeItem = async (it: SupplierType) => {
    try {
      ...
      showToast(e?.message || "Erro ao remover tipo de fornecedor", "error");
    }
  };
      ...
      <div className="w-full max-w-3xl max-h-[95vh] overflow-hidden bg-white shadow-2xl rounded-2xl border border-gray-200 flex flex-col">
        <div className="bg-gradient-to-r from-orange-50 to-amber-50 border-b border-gray-200 sticky top-0 z-20 relative p-6">
          ...
          <div className="relative flex items-center justify-between">
            <div>
              <h2 className="text-xl font-bold text-gray-900">Gerenciar Tipos de Fornecedor</h2>
        ...
        <div className="fixed inset-0 z-[60] bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-md overflow-hidden bg-white shadow-2xl rounded-2xl border border-gray-200 flex flex-col">
            <div className="bg-gradient-to-r from-orange-50 to-amber-50 border-b border-gray-200 sticky top-0 z-20 relative p-5 flex items-center justify-between">
              <h3 className="text-lg font-semibold text-gray-900">{editing ? "Editar" : "Novo"} Tipo de Fornecedor</h3>
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="flex items-center gap-2">
                <input type="checkbox" id="st-active" checked={active} onChange={e => setActive(e.target.checked)} className="rounded border-border" />
                <label htmlFor="st-active" className="text-sm font-medium">Ativo</label>
              </div>
            </div>
